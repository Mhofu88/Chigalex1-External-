...
const LANGUAGES={"en": "English", "fr": "Français", "pt": "Português", "sw": "Kiswahili", "sn": "Shona - ChiShona", "nd": "Ndebele - isiNdebele", "zu": "Zulu", "ar": "العربية - Arabic", "af": "Afrikaans", "ha": "Hausa - Hausa", "yo": "Yorùbá - Yoruba", "ig": "Igbo - Igbo", "tw": "Twi - Akan", "wo": "Wolof", "am": "አማርኛ - Amharic", "rw": "Kinyarwanda", "lg": "Luganda", "so": "Somali", "zgh": "Tamazight ⵜⴰⵎⴰⵣⵉⵖⵜ", "ln": "Lingala", "kg": "Kikongo", "xh": "isiXhosa - Xhosa", "tn": "Setswana", "st": "Sesotho - Sotho", "ts": "Xitsonga - Shangani", "ny": "Chichewa - Nyanja", "se": "Sena", "ss": "siSwati", "kj": "Oshiwambo", "bem": "Bemba", "ce": "Chewa", "ve": "Tshivenda - Venda", "nr": "Ndebele South (alias)", "st2": "Sesotho duplicate"};
...
async function loadLang(lang){
  ...
  try{
    ...
    dict=await res.json();
    console.log('Loaded',lang, Object.keys(dict).length+' keys');
  }catch(e){
    console.error('Failed load',lang,e);
    // try fallback folder
    try{
      const res2=await fetch('./locales/'+lang.split('_')[0]+'/translation.json');
      if(res2.ok) dict=await res2.json();
    }catch(_){
      dict={};
    }
  }
...
function renderSheet(){
  ...
  if(!sheet) return;
  let html='<div style="position:sticky;top:0;background:#fff;padding:10px 0;z-index:1;border-bottom:1px solid #eee"><h3>🌍 Pan African - Select Language</h3><input id="langSearch" placeholder="Search language..." onkeyup="filterLang()" style="width:100%;padding:8px 12px;border-radius:8px;border:1px solid #ddd;margin-top:8px"></div>';
